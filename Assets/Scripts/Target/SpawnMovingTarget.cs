...
public class SpawnMovingTarget : MonoBehaviour {
    ...
    public float Timer;
    public float PITCH, YAW, DotProduct;
    void CreateCube()
    {
	...
	void Update () {
        ...
        DotProduct = VectorMaths.DotProduct(ForwardDirection, FindObjectOfType<Sniper>().ForwardDirection );
        if (VectorMaths.DotProduct(ForwardDirection, FindObjectOfType<Sniper>().ForwardDirection) < 0)
        {
            if (Input.GetKey(KeyCode.E))
            {
                Playercontrol = true;
